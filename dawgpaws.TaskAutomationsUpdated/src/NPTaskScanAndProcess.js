...
export function getSingleTaskOptions(origPara: TParagraph, todayLines: any, content: string): Array<{ label: string, value: string }> {
  const opts = [
    ...
    { label: `📄 Move to another note`, value: '__moveNote__' },
...
export async function processUserAction(origPara: TParagraph, optionChosen: CommandBarChoice): Promise<number> {
  ...
  switch (userChoice) {
    ...
    case '__moveNote__':
      return await handleMoveParagraphToNoteAction(origPara)
    case '__remove__':
...
/**
 * Move a paragraph to another note, preserving its type.
 * @param {TParagraph} origPara
 * @returns {Promise<number>} CONTINUE if moved, SEE_TASK_AGAIN if not
 */
export async function handleMoveParagraphToNoteAction(origPara: TParagraph): Promise<number> {
  const noteToMoveTo = await chooseNote(true, true, [], 'Note to move to', true, true)
  if (noteToMoveTo) {
    const heading = await chooseHeading(noteToMoveTo, true, true, false)
    if (heading) {
      ...
      logError(pluginJson, `handleMoveParagraphToNoteAction: could not find heading in note: ${noteToMoveTo.title || ''}`)
      return SEE_TASK_AGAIN
    }
  } else {
    logError(pluginJson, `handleMoveParagraphToNoteAction: could not find note to move to`)
    return SEE_TASK_AGAIN
  }
}
