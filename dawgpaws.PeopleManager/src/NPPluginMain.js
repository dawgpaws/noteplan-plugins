...
// `noteplan-cli plugin:dev dawgpaws.PeopleManager --test --watch --coverage`
...
export async function managePeopleNotes(): Promise<void> {
  try {
    const { peopleFolder, peopleTemplate, personPrefix } = DataStore.settings

    if (!peopleFolder || !peopleTemplate) {
      await CommandBar.prompt('⚠️ Please configure the plugin settings first.')
      return
    }

    // Load all mentions known to NotePlan
    const mentions = DataStore.mentions || []
    if (mentions.length === 0) {
      await CommandBar.prompt('No mentions found in your notes.')
      return
    } else {
